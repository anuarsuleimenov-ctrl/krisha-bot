...
async def get_best_price_per_zhk(zhk_names: list[str]) -> dict:
    """Return dict {zhk_name: min_price_m2} for active validated listings."""
    if not zhk_names:
        return {}
    async with pool.acquire() as conn:
        placeholders = ",".join(f"${i+1}" for i in range(len(zhk_names)))
        rows = await conn.fetch(
            f"""SELECT zhk, MIN(price_m2) as min_price_m2
               FROM listings
               WHERE is_active = TRUE
                 AND validation IN ('✅ ОК', '📞 Уточнено')
                 AND zhk IN ({placeholders})
               GROUP BY zhk""",
            *zhk_names
        )
        return {row["zhk"]: row["min_price_m2"] for row in rows}
