...
async def cmd_start(msg: Message):
    ...
    if not user:
        ...
        await db.add_subscription(
            ...
            amount=0,
            method="Авто",
            start=datetime.now(ASTANA_TZ).date(),
            end=end,
...
async def cmd_top(msg: Message):
    ...
    # Check if listing is the best price in its ZhK across ALL active listings
    zhk_names = [l["zhk"] for l in sorted_listings]
    best_prices = await db.get_best_price_per_zhk(zhk_names)
    ...
    for i, l in enumerate(sorted_listings, 1):
        ...
        # Best price label
        best_label = ""
        if l["zhk"] in best_prices and l["price_m2"] <= best_prices[l["zhk"]]:
            best_label = "🏷 *лучшая цена в данном ЖК*\n"
        cards.append(
            f"🏆 *#{i}* | {l['district']}\n"
            f"🏗 ЖК: {l['zhk']}\n"
            f"{best_label}"
...
async def scheduled_parse_and_send():
    ...
    for user in users:
        try:
            ...
            # Get best prices across all active listings
            zhk_names = [l["zhk"] for l in top]
            best_prices = await db.get_best_price_per_zhk(zhk_names)
